...
public class DriveSubsystem extends SubsystemBase {
  ...
  public DriveSubsystem() {
	...
	private final SlewRateLimiter yAccelFilter = new SlewRateLimiter(Constants.DRIVE_ACCELERATION_LIMIT);
	private final SlewRateLimiter xAccelFilter = new SlewRateLimiter(Constants.DRIVE_ACCELERATION_LIMIT);
	private final SlewRateLimiter rotationAccelFilter = new SlewRateLimiter(Constants.DRIVE_ROTATION_ACCELERATION_LIMIT);
	...
	public void stop() {
		...
		yAccelFilter.reset(0);
   	 	xAccelFilter.reset(0);
		rotationAccelFilter.reset(0);
	}
	...
	public void testone() {
		...
		double leftTriggerValue = c.getLeftTriggerAxis();
	
		SmartDashboard.putNumber("TriggerValue", leftTriggerValue);
		if (leftTriggerValue > 0) {
			setLeftMotorSpeed(leftTriggerValue);
			setRightMotorSpeed(leftTriggerValue * -1);
		}
	}
	@Override
	...
	public void driveCartesian(double ySpeed, double xSpeed, double zRotation) {
		robotDrive.driveCartesian(
			yAccelFilter.calculate(ySpeed),
			xAccelFilter.calculate(xSpeed),
			rotationAccelFilter.calculate(zRotation)
		);
	}
